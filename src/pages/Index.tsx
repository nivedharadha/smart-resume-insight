  ...
  const handleAnalyze = async () => {
    ...
    if (jobDescription.length > 10000) {
      toast({
        ...
        description: "Please limit job description to 10,000 characters.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // Save to database - access_token is auto-generated by the database
      // Store the storage file name (not the signed URL) for security
      const { data: record, error: dbError } = await supabase
        .from("analysis_records")
        .insert({
          resume_file_name: fileName, // Store the storage file name for on-demand URL generation
          resume_file_url: fileName, // Use file name as reference (signed URLs generated on-demand)
      ...
      // Navigate to results with the access token for secure viewing
      navigate(`/results/${record.id}?token=${record.access_token}`);
    } catch (error) {
